...
function DemoNodeInner({ data }: NodeProps<DemoNodeType>) {
  const { step, title, sub, active, done } = data;
  ...
  return (
    <div
      ...
      className={cn(
        ...
        done && !active && 'border-emerald-800/70 bg-emerald-950/20 text-zinc-300',
        idle && 'border-zinc-800 bg-zinc-900/50 text-zinc-500 opacity-60',
      )}
    >
      ...
      <Handle
        ...
        className={cn('!h-2.5 !w-2.5 !border-none', lit ? '!bg-emerald-400' : '!bg-zinc-600')}
      />
        ...
        <div className="min-w-0">
          <p
            className={cn(
              ...
              active ? 'text-emerald-200' : done ? 'text-zinc-200' : 'text-zinc-500',
            )}
          >
            {title}
          </p>
          ...
          <p
            className={cn(
              ...
              active ? 'text-emerald-300/90' : done ? 'text-zinc-400' : 'text-zinc-600',
            )}
          >
            {sub}
          </p>
