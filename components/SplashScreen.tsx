...
const fadeUp = {
  ...
  exit: { opacity: 0, y: -22, transition: { duration: 0.4, ease: 'easeIn' as const } },
};
    ...
    >
      {MARQUEE.map((label, i) => (
        <li key={i} className="flex items-center gap-8">
          ...
          <span aria-hidden="true" className="h-1.5 w-1.5 rotate-45 bg-mint/30" />
        </li>
      ))}
    </ul>
  );
...
export default function SplashScreen({ onFinish }: SplashScreenProps) {
  ...
  return (
    <motion.div
      role="dialog"
      aria-modal="true"
      aria-label="180 Degrees Consulting presents"
      onClick={finish}
      className="fixed inset-0 z-[100] flex cursor-pointer flex-col overflow-hidden bg-navy text-white"
      initial={{ opacity: 0 }}
    ...
    >
      ...
      <div
        ...
        style={{ background: 'radial-gradient(circle, rgb(135 180 60 / 0.14), transparent 70%)' }}
      />
      ...
      <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-5 text-center">
        ...
        >
          180 Degrees Consulting presents
        </motion.p>
        ...
        >
          A 180° on the{' '}
          <span className="text-mint">Model Context Protocol</span>
        </motion.h1>
        ...
        >
          Every problem has a flip side. Built by 180DC, the student consultancy that
          turns stale angles into fresh ones, to explain the why and the what of MCP.
        </motion.p>
      </div>
      ...
      <div className="relative z-10 flex flex-1 items-end justify-center pb-8">
        ...
        >
          <span>180DC</span>
          <span className="h-1 w-1 rotate-45 bg-mint/30" />
          <span>Discover</span>
          <span className="h-1 w-1 rotate-45 bg-mint/30" />
          <span>Develop</span>
          <span className="h-1 w-1 rotate-45 bg-mint/30" />
